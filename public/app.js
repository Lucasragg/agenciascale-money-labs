const state={data:null,start:null,end:null,level:'campaign',view:'Bubba'};
...
const inView=x=>x.view===state.view;
...
function aggregate(start,end,group){
  ...
  state.data.ads.filter(x=>inView(x)&&inRange(x.date,start,end)).forEach(x=>{const v=take(group?keyOf(x):'_');v.spend+=x.spend;v.impressions+=x.impressions;v.clicks+=x.clicks;v.pageViews+=x.pageViews});
  state.data.events.filter(x=>inView(x)&&inRange(x.date,start,end)).forEach(x=>{const v=take(group?keyOf(x):'_');if(x.type==='lead')v.leads++;else v.sales++});
...
function dailyData(){const map=new Map();for(let d=state.start;d<=state.end;d=shift(d,1))map.set(d,{date:d,spend:0,leads:0,sales:0});state.data.ads.filter(x=>inView(x)&&map.has(x.date)).forEach(x=>map.get(x.date).spend+=x.spend);state.data.events.filter(x=>inView(x)&&map.has(x.date)).forEach(x=>{const v=map.get(x.date);v[x.type==='lead'?'leads':'sales']++});return [...map.values()]}
...
function renderTable(){const rows=aggregate(state.start,state.end,state.level).sort((a,b)=>{const ac=a.sales?ratio(a.spend,a.sales):Infinity,bc=b.sales?ratio(b.spend,b.sales):Infinity;return ac-bc||b.leads-a.leads});$('#optimizationBody').innerHTML=rows.length?rows.map(x=>`<tr><td title="${escapeHtml(x.name)}">${escapeHtml(x.name)}</td><td>${money(x.spend)}</td><td>${num(x.clicks)}</td><td>${num(x.leads)}</td><td>${x.leads?money(x.spend/x.leads):'—'}</td><td>${num(x.sales)}</td><td>${x.sales?money(x.spend/x.sales):'—'}</td></tr>`).join(''):'<tr><td colspan="7" class="empty">Sem dados atribuídos no período.</td></tr>'}
function escapeHtml(s){return String(s).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
function viewRange(){const dates=[...state.data.ads.filter(inView),...state.data.events.filter(inView)].map(x=>x.date).sort();return{min:dates[0]||state.data.cutoffDate,max:dates.at(-1)||state.data.cutoffDate}}
...
function render(){renderMetrics();renderFunnel();renderChart();renderTable();renderCoverage();$('#periodLabel').textContent=`${dateBR(state.start)} — ${dateBR(state.end)}`;document.title=`${state.view} · Money Labs Meta Ads`}
function setPeriod(days){const range=viewRange(),max=range.max,min=range.min;state.end=max;state.start=days==='all'?min:[shift(max,-Number(days)+1),min].sort().at(-1);$('#startDate').min=min;$('#startDate').max=max;$('#endDate').min=min;$('#endDate').max=max;$('#startDate').value=state.start;$('#endDate').value=state.end;render()}

async function init(){try{const res=await fetch(`data.json?v=${Date.now()}`,{cache:'no-store'});if(!res.ok)throw new Error(`HTTP ${res.status}`);state.data=await res.json();const requestedView=new URLSearchParams(location.search).get('view');if(state.data.views.includes(requestedView))state.view=requestedView;$$('#viewTabs button').forEach(x=>x.classList.toggle('active',x.dataset.view===state.view));$('#updatedAt').textContent=new Intl.DateTimeFormat('pt-BR',{dateStyle:'short',timeStyle:'short',timeZone:'America/Sao_Paulo'}).format(new Date(state.data.generatedAt));$('#taxRate').textContent=num(state.data.taxMultiplier,4);$('#exchangeRate').textContent=`R$ ${num(state.data.brlPerUsd,2)} por US$ 1`;setPeriod(7);
  $('#viewTabs').addEventListener('click',e=>{if(!e.target.dataset.view)return;state.view=e.target.dataset.view;$$('#viewTabs button').forEach(x=>x.classList.toggle('active',x===e.target));const activeDays=$('#quickFilters button.active')?.dataset.days||7;setPeriod(activeDays)});$('#quickFilters').addEventListener('click',e=>{if(!e.target.dataset.days)return;$$('#quickFilters button').forEach(x=>x.classList.toggle('active',x===e.target));setPeriod(e.target.dataset.days)});['startDate','endDate'].forEach(id=>$('#'+id).addEventListener('change',()=>{state.start=$('#startDate').value;state.end=$('#endDate').value;if(state.start>state.end)[state.start,state.end]=[state.end,state.start];$$('#quickFilters button').forEach(x=>x.classList.remove('active'));render()}));$('#tabs').addEventListener('click',e=>{if(!e.target.dataset.level)return;state.level=e.target.dataset.level;$$('#tabs button').forEach(x=>x.classList.toggle('active',x===e.target));renderTable()});
