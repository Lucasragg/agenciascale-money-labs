...
function aggregate(start,end,group){
  ...
  state.data.events.filter(x=>inRange(x.date,start,end)).forEach(x=>{const v=take(group?keyOf(x):'_');if(x.type==='lead')v.leads++;else v.sales++});
